...
export var renderTemplFull = (files: R2Object[], folders: string[], path: string, config: SiteConfig) => {
    ...
    <html>
    <head>
        <link rel="icon" href="${config.favicon}" type="${config.faviconType}">
        <meta charset="utf-8">
...
var renderTemplFolders = (folders: string[], siteConfig: SiteConfig) => {
	...
	for (var i = 0; i < folders.length; i++) {
		output += `<tr class="file ">
                            ...
                            <td class="description">${findDescription(siteConfig, '/' + folders[i].slice(0, -1), true) ?? '&mdash;'}</td>
                            <td class="size">&mdash;</td>
                            <td class="date hideable">&mdash;</td>
                            <td class="hideable"></td>
                        </tr>`;
...
var renderTemplFiles = (files: R2Object[], siteConfig: SiteConfig) => {
	...
	for (var i = 0; i < files.length; i++) {
		output += `<tr class="file ">
                            ...
                            <td class="description">${findDescription(siteConfig, '/' + files[i].key, true) ?? '&mdash;'}</td>
...
function findDescription(siteConfig: SiteConfig, path: string, exact: boolean): string | undefined {
	if (exact) {
		return siteConfig.descriptions[path];
	}
	const keys = Object.keys(siteConfig.descriptions);
	...
	const description = siteConfig.descriptions[longestMatch];
	return description;
}

function generateFooter(siteConfig: SiteConfig, path: string): string {
	let contents: string[] = [];
	const description = findDescription(siteConfig, path, false);
	if (description) {
		contents.push(`<p>${description}</p>`);
	}
